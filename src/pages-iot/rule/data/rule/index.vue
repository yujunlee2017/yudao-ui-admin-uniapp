<template>
  <view class="yd-page-container yd-page-container-paging">
    <!-- 顶部导航栏 -->
    <wd-navbar title="数据规则" left-arrow placeholder safe-area-inset-top fixed @click-left="handleBack" />

    <!-- 搜索组件 -->
    <SearchForm @search="handleQuery" @reset="handleReset" />

    <!-- 数据规则列表 -->
    <z-paging ref="pagingRef" v-model="list" :fixed="false" class="min-h-0 flex-1" :default-page-size="10" :refresher-enabled="true" :inside-more="true" :loading-more-default-as-loading="true" empty-view-text="暂无数据规则数据" @query="queryList">
      <view class="p-24rpx">
        <view v-for="item in list" :key="item.id" class="mb-24rpx rounded-12rpx bg-white p-24rpx shadow-sm" @click="handleDetail(item)">
          ...
          <view class="mb-12rpx text-28rpx text-[#666]"><text class="mr-8rpx text-[#999]">数据源：</text>{{ item.sourceConfigs?.length || 0 }} 个</view>
          <view class="mb-12rpx text-28rpx text-[#666]"><text class="mr-8rpx text-[#999]">数据目的：</text>{{ item.sinkIds?.length || 0 }} 个</view>
    ...
    <!-- 新增按钮 -->
    <wd-fab v-if="hasAccessByCodes(['iot:data-rule:create'])" position="right-bottom" type="primary" :expandable="false" @click="handleAdd" />
  </view>
</template>
...
/** 卸载 */
onUnload(() => { uni.$off('iot:data-rule:reload', reload) })
</script>
