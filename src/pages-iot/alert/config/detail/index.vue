  ...
  <view class="yd-page-container">
    ...
    <view class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        <wd-button v-if="hasAccessByCodes(['iot:alert-config:update'])" class="flex-1" type="warning" @click="handleEdit">
          编辑
        </wd-button><wd-button v-if="hasAccessByCodes(['iot:alert-config:delete'])" class="flex-1" type="danger" :loading="deleting" @click="handleDelete">
          删除
        </wd-button>
      </view>
    </view>
...
async function handleDelete() {
  ...
  deleting.value = true
  try { await deleteAlertConfig(Number(props.id)); toast.success('删除成功'); uni.$emit('iot:alert-config:reload'); delay(handleBack) } finally { deleting.value = false }
}
