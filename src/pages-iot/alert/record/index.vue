    ...
    <z-paging ref="pagingRef" v-model="list" :fixed="false" class="min-h-0 flex-1" :default-page-size="10" :refresher-enabled="true" :inside-more="true" :loading-more-default-as-loading="true" empty-view-text="暂无告警记录数据" @query="queryList">
      <view class="p-24rpx">
        <view v-for="item in list" :key="item.id" class="mb-24rpx rounded-12rpx bg-white p-24rpx shadow-sm" @click="handleDetail(item)">
          ...
          <view class="mb-12rpx text-28rpx text-[#666]"><text class="mr-8rpx text-[#999]">产品：</text>{{ productLabel(item) }}</view>
          <view class="mb-12rpx text-28rpx text-[#666]"><text class="mr-8rpx text-[#999]">设备：</text>{{ deviceLabel(item) }}</view>
...
/** 产品名称（记录仅返回 ID，本地解析） */
function productLabel(record: AlertRecord) { return record.productId != null ? productMap.value[String(record.productId)] || String(record.productId) : '-' }

/** 设备名称（记录仅返回 ID，本地解析） */
function deviceLabel(record: AlertRecord) { return record.deviceId != null ? deviceMap.value[String(record.deviceId)] || String(record.deviceId) : '-' }
