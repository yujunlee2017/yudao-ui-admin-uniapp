  ...
  <view class="yd-page-container pb-120rpx">
    ...
    <view class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        ...
        <wd-button
          v-if="canDelete && hasAccessByCodes(['wms:movement-order:delete'])"
...
const moreActions = computed(() => {
  ...
  const actions = []
  if (hasAccessByCodes(['wms:movement-order:complete'])) {
    actions.push({ name: '完成移库', value: 'complete' })
  }
  if (hasAccessByCodes(['wms:movement-order:cancel'])) {
    actions.push({ name: '作废移库单', value: 'cancel' })
  }
  return actions
})
