  ...
  <view class="yd-page-container">
    ...
    <view v-if="canUpdate || canDelete" class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        ...
        <wd-button v-if="canUpdate && formData.status === CommonStatusEnum.DISABLE" class="flex-1" type="primary" :loading="submitting" @click="() => handleStatus(CommonStatusEnum.ENABLE)">
          启用
        </wd-button>
        <wd-button v-if="canUpdate && formData.status === CommonStatusEnum.ENABLE" class="flex-1" type="info" :loading="submitting" @click="() => handleStatus(CommonStatusEnum.DISABLE)">
          禁用
        </wd-button>
...
const canUpdate = computed(() => hasAccessByCodes(['promotion:coupon-template:update']))
const canDelete = computed(() => hasAccessByCodes(['promotion:coupon-template:delete']))
