...
async function queryList(pageNo: number, pageSize: number) {
  try {
    const data = await getProductFavoritePage({ ...queryParams.value, pageNo, pageSize })
...
/** 查看详情：收藏接口无 get 接口，详情字段经路由参数透传，避免“取前 100 条再 find”的反模式 */
function handleDetail(item: ProductFavorite) {
  const query = [
    `id=${item.id}`,
  ...
  uni.navigateTo({ url: `/pages-mall/product/favorite/detail/index?${query}` })
}

/** 初始化 */
onMounted(() => {
  uni.$on('mall:product-favorite:reload', reload)
})

/** 卸载 */
onUnload(() => {
  uni.$off('mall:product-favorite:reload', reload)
})
</script>
