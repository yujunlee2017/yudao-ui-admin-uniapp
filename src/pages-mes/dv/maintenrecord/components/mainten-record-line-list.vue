      ...
      <scroll-view class="min-h-0 flex-1" scroll-y>
        <wd-form ref="formRef" :model="formData" :schema="formSchema">
          <wd-cell-group border>
            <wd-form-item title="保养项目" title-width="200rpx" prop="subjectId">
              <view class="min-h-56rpx flex items-center justify-between rounded-8rpx px-4rpx" @click.stop="openSubjectSelector">
                <text :class="selectedSubjectText ? 'text-[#333]' : 'text-[#999]'">
                  {{ selectedSubjectText || '请选择保养项目' }}
                </text>
                <wd-icon name="arrow-right" size="28rpx" color="#999" />
              </view>
            </wd-form-item>
            <wd-form-item title="保养结果" title-width="200rpx" prop="status">
              <wd-radio-group v-model="formData.status" type="button">
                <wd-radio v-for="dict in getIntDictOptions(DICT_TYPE.MES_MAINTEN_STATUS)" :key="dict.value" :value="dict.value">
                  {{ dict.label }}
                </wd-radio>
              </wd-radio-group>
            </wd-form-item>
...
import type {
  DvMaintenRecordLineCreateReqVO,
  DvMaintenRecordLineVO,
...
import { DICT_TYPE, MesDvMaintenStatusEnum } from '@/utils/constants'
...
interface DvMaintenRecordLineFormData extends DvMaintenRecordLineCreateReqVO {
  id?: number
}
...
const list = ref<DvMaintenRecordLineVO[]>([]) // 明细列表
const formVisible = ref(false) // 明细表单显示状态
const formLoading = ref(false) // 表单提交状态
const formRef = ref<FormInstance>() // 表单引用
const formData = ref<DvMaintenRecordLineFormData>(getDefaultFormData()) // 表单数据
const selectedSubject = ref<DvSubjectVO>() // 当前选择保养项目
...
function getDefaultFormData() {
  return {
    ...
    result: '',
    remark: '',
  } as DvMaintenRecordLineFormData
}
...
/** 打开编辑表单 */
function openUpdateForm(item: DvMaintenRecordLineVO) {
  formData.value = {
    id: item.id,
...
async function handleSubmit() {
  ...
  try {
    const data: DvMaintenRecordLineCreateReqVO = {
      recordId: props.recordId,
      subjectId: formData.value.subjectId,
      status: formData.value.status,
      result: formData.value.result || undefined,
      remark: formData.value.remark || undefined,
    }
    if (formData.value.id) {
      await updateMaintenRecordLine({ ...data, id: formData.value.id })
      toast.success('修改成功')
    } else {
      await createMaintenRecordLine(data)
      toast.success('添加成功')
    }
...
/** 删除明细 */
async function handleDelete(item: DvMaintenRecordLineVO) {
  try {
