          ...
          <view class="p-24rpx">
            <view class="mb-16rpx flex items-start justify-between gap-16rpx">
              <view class="min-w-0 flex-1 truncate text-32rpx text-[#333] font-semibold">
                {{ item.name || '-' }}
              </view><dict-tag :type="DICT_TYPE.COMMON_STATUS" :value="item.status" />
            </view>
            <view class="text-26rpx text-[#666] space-y-8rpx">
              <view>编码：{{ item.code || '-' }}</view><view v-if="item.nickname">
                简称：{{ item.nickname }}
              </view>
              <view class="flex items-center">
                <text class="mr-8rpx">类型：</text><dict-tag v-if="item.type != null" :type="DICT_TYPE.MES_CLIENT_TYPE" :value="item.type" /><text v-else>-</text>
              </view>
              <view>电话：{{ item.telephone || '-' }}</view><view v-if="item.remark">
                备注：{{ item.remark }}
              </view>
            </view>
          </view>
...
import type { MdClientQueryParams, MdClientVO } from '@/api/mes/md/client'
...
function handleQuery(data: MdClientQueryParams) {
...
function handleDetail(item: MdClientVO) {
