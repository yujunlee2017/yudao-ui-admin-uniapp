<template>
  <view class="yd-page-container yd-page-container-paging">
    ...
    <view v-if="hasAccessByCodes(['mes:md-workshop:export'])" class="bg-white px-24rpx py-16rpx">
    ...
    <z-paging ref="pagingRef" v-model="list" :fixed="false" class="min-h-0 flex-1" :default-page-size="10" :refresher-enabled="true" :inside-more="true" :loading-more-default-as-loading="true" empty-view-text="暂无车间数据" @query="queryList">
      <view class="p-24rpx">
    ...
    <wd-fab v-if="hasAccessByCodes(['mes:md-workshop:create'])" position="right-bottom" type="primary" :expandable="false" @click="handleAdd" />
  </view>
</template>

<script lang="ts" setup>
import type { MdWorkshopQueryParams, MdWorkshopVO } from '@/api/mes/md/workstation/workshop'
import { onUnload } from '@dcloudio/uni-app'
import { onMounted, ref } from 'vue'
import { deleteWorkshop, getWorkshopPage } from '@/api/mes/md/workstation/workshop'
...
const list = ref<MdWorkshopVO[]>([])
const pagingRef = ref<ZPagingRef<MdWorkshopVO>>()
const queryParams = ref<MdWorkshopQueryParams>({})
...
const {
  ...
  deleteApi: (ids: number[]) => Promise.all(ids.map(id => deleteWorkshop(id))).then(() => {}),
  reloadEvent: 'mes:md:workshop:reload',
})
...
function handleQuery(data: MdWorkshopQueryParams) {
...
async function handleExport() {
  ...
  try {
    await downloadApiFile(`/mes/md-workshop/export-excel`, queryParams.value, '车间.xls')
  } finally {
...
function handleDetail(item: MdWorkshopVO) {
