<template>
  ...
  >
    ...
    >
      ...
      <view class="mb-8rpx flex text-26rpx text-[#666]">
        <text class="mr-8rpx shrink-0 text-[#999]">发料数量：</text>
...
import type {
  WmOutsourceIssueDetailCreateReqVO,
  WmOutsourceIssueDetailVO,
} from '@/api/mes/wm/outsourceissue/detail'
import type {
  WmOutsourceIssueLineCreateReqVO,
  WmOutsourceIssueLineVO,
...
interface WmOutsourceIssueLineFormData extends WmOutsourceIssueLineCreateReqVO {
  id?: number
}

interface WmOutsourceIssueDetailFormData extends WmOutsourceIssueDetailCreateReqVO {
  id?: number
...
const list = ref<WmOutsourceIssueLineVO[]>([]) // 发料物料列表
const formVisible = ref(false) // 行表单显示状态
const formLoading = ref(false) // 表单提交状态
const formRef = ref<FormInstance>() // 表单引用
const formData = ref<WmOutsourceIssueLineFormData>(getDefaultFormData()) // 表单数据
...
const detailFormData = ref<WmOutsourceIssueDetailFormData>(getDefaultDetailFormData()) // 拣货明细表单数据
...
const detailListMap = ref<Record<number, WmOutsourceIssueDetailVO[]>>({}) // 拣货明细列表
...
/** 默认表单数据 */
function getDefaultFormData(): WmOutsourceIssueLineFormData {
  return {
...
/** 默认拣货明细表单数据 */
function getDefaultDetailFormData(): WmOutsourceIssueDetailFormData {
  return {
    issueId: props.issueId,
    lineId: undefined,
    materialStockId: undefined,
...
/** 打开编辑表单 */
function openUpdateForm(item: WmOutsourceIssueLineVO) {
  formData.value = {
    id: item.id,
...
async function handleSubmit() {
  ...
  try {
    const data: WmOutsourceIssueLineCreateReqVO = {
      issueId: props.issueId,
    ...
    if (formData.value.id) {
      await updateOutsourceIssueLine({ ...data, id: formData.value.id })
      toast.success('修改成功')
    } else {
      await createOutsourceIssueLine(data)
      toast.success('添加成功')
    }
...
/** 删除发料物料 */
async function handleDelete(item: WmOutsourceIssueLineVO) {
  try {
...
/** 打开新增拣货明细表单 */
function openCreateDetailForm(item: WmOutsourceIssueLineVO) {
...
/** 打开编辑拣货明细表单 */
function openUpdateDetailForm(item: WmOutsourceIssueLineVO, detail: WmOutsourceIssueDetailVO) {
  detailFormData.value = {
    id: detail.id,
...
/** 删除拣货明细 */
async function handleDeleteDetail(detail: WmOutsourceIssueDetailVO) {
  try {
    await dialog.confirm({
      title: '提示',
...
async function handleSubmitDetail() {
  ...
  try {
    const data: WmOutsourceIssueDetailCreateReqVO = {
      issueId: props.issueId,
    ...
    if (detailFormData.value.id) {
      await updateOutsourceIssueDetail({ ...data, id: detailFormData.value.id })
      toast.success('修改成功')
    } else {
      await createOutsourceIssueDetail(data)
      toast.success('新增成功')
    }
