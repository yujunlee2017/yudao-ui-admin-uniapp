      ...
      <view v-if="items.length > 0" class="mt-24rpx">
        ...
        <view class="px-24rpx">
          ...
          >
            ...
            <view v-if="item.inCount != null && item.inCount !== ''" class="mb-10rpx flex text-26rpx text-[#666]">
              <text class="mr-8rpx shrink-0 text-[#999]">已入库：</text>
              <text class="min-w-0 flex-1">{{ formatCount(item.inCount) }}</text>
            </view>
            <view v-if="item.returnCount != null && item.returnCount !== ''" class="mb-10rpx flex text-26rpx text-[#666]">
...
async function getDetail() {
  ...
  try {
    toast.loading('加载中...')
    formData.value = await enrichErpDocumentDetail(await getPurchaseReturn(props.id), 'purchase-return') as PurchaseReturn
  } finally {
    toast.close()
  }
}
