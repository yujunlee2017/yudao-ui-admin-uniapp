  ...
  <view class="yd-page-container">
    ...
    <view v-if="hasFooter" class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        ...
        <wd-button v-if="hasAccessByCodes(['erp:sale-out:delete'])" class="flex-1" type="danger" :loading="deleting" @click="handleDelete">
          删除
        </wd-button>
      </view>
    </view>
...
const hasFooter = computed(() => canUpdate.value || hasAccessByCodes(['erp:sale-out:delete']) || canUpdateStatus.value)
...
async function getDetail() {
  ...
  try {
    toast.loading('加载中...')
    formData.value = await enrichErpDocumentDetail(await getSaleOut(Number(props.id)), 'sale-out')
  } finally {
    toast.close()
  }
}
...
async function handleUpdateStatus(status: number) {
  ...
  try {
    await updateSaleOutStatus(Number(props.id), status)
