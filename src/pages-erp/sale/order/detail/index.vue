    ...
    <view v-if="hasFooter" class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        ...
        <wd-button
          ...
          class="flex-1" type="primary" :loading="statusLoading" @click="handleUpdateStatus(nextStatus)"
        >
...
async function getDetail() {
  ...
  try {
    toast.loading('加载中...')
    formData.value = await enrichErpDocumentDetail(await getSaleOrder(Number(props.id)), 'sale-order')
  } finally {
    toast.close()
  }
}
...
async function handleDelete() {
  ...
  deleting.value = true
  try {
    await deleteSaleOrder([Number(props.id)])
    toast.success('删除成功')
...
async function handleUpdateStatus(status: number) {
  ...
  try {
    await updateSaleOrderStatus(Number(props.id), status)
