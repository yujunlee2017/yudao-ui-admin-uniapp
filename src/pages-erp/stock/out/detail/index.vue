...
async function getDetail() {
  ...
  try {
    toast.loading('加载中...')
    formData.value = await enrichErpDocumentDetail(await getStockOut(Number(currentId.value)), 'stock-out') as StockOut
  } finally {
    toast.close()
  }
}
...
/** 编辑其它出库 */
function handleEdit() {
  uni.navigateTo({ url: `/pages-erp/stock/out/form/index?id=${currentId.value}` })
}
...
async function handleDelete() {
  ...
  deleting.value = true
  try {
    await deleteStockOut([Number(currentId.value)])
    toast.success('删除成功')
...
async function handleUpdateStatus(status: number) {
  ...
  try {
    await updateStockOutStatus(Number(currentId.value), status)
