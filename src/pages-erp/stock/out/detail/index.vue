  ...
  <view class="yd-page-container">
    ...
    <view v-if="hasFooter" class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        ...
        <wd-button v-if="hasAccessByCodes(['erp:stock-out:delete'])" class="flex-1" type="danger" :loading="deleting" @click="handleDelete">
          删除
        </wd-button>
      </view>
    </view>
...
const hasFooter = computed(() => canUpdate.value || hasAccessByCodes(['erp:stock-out:delete']) || canUpdateStatus.value)
...
async function getDetail() {
  ...
  try {
    toast.loading('加载中...')
    formData.value = await enrichErpDocumentDetail(await getStockOut(Number(props.id)), 'stock-out') as StockOut
  } finally {
    toast.close()
  }
}
...
async function handleDelete() {
  ...
  deleting.value = true
  try {
    await deleteStockOut([Number(props.id)])
    toast.success('删除成功')
...
async function handleUpdateStatus(status: number) {
  ...
  try {
    await updateStockOutStatus(Number(props.id), status)
