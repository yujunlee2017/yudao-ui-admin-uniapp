        ...
        <view class="yd-search-form-date-range-container">
          <view class="flex-1" @click="dateVisible.start = true">
            <view class="yd-search-form-date-range-picker">
              {{ formatDate(formData.moveTime[0]) || '开始日期' }}
            </view>
          </view>
          -
          <view class="flex-1" @click="dateVisible.end = true">
            <view class="yd-search-form-date-range-picker">
              {{ formatDate(formData.moveTime[1]) || '结束日期' }}
            </view>
          </view>
        </view>
        <wd-datetime-picker v-model="formData.moveTime[0]" v-model:visible="dateVisible.start" title="请选择开始日期" type="date" />
        <wd-datetime-picker v-model="formData.moveTime[1]" v-model:visible="dateVisible.end" title="请选择结束日期" type="date" />
      </view>
      <view class="yd-search-form-item">
        <view class="yd-search-form-label">
          仓库
        </view>
        <wd-form-item :value="getPickerDisplay(warehouseOptions, formData.fromWarehouseId)" placeholder="请选择仓库" is-link @click="pickerVisible.warehouse = true" />
        <wd-picker v-model:visible="pickerVisible.warehouse" :model-value="formData.fromWarehouseId" :columns="warehouseOptions" label-key="name" value-key="id" @confirm="({ value }) => formData.fromWarehouseId = value[0]" />
      </view>
      <view class="yd-search-form-item">
        <view class="yd-search-form-label">
          创建人
...
const placeholder = computed(() => {
  ...
  if (formData.fromWarehouseId) {
    conditions.push(`仓库:${getPickerDisplay(warehouseOptions.value, formData.fromWarehouseId)}`)
  }
