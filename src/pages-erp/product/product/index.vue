...
import { deleteProduct, getProductPage } from '@/api/erp/product/product'
...
const {
  ...
  deleteApi: (ids: number[]) => deleteProduct(ids[0]), // TODO @Yunai：BUG！ids[0] 只删第一条（后端删除接口只接单 id）。改为 for 循环逐条删，参考 finance/account/index.vue 或 product/category/index.vue:148。
  reloadEvent: 'erp:product:reload',
})
...
/** 新增产品 */
function handleAdd() {
  uni.navigateTo({ url: '/pages-erp/product/product/form/index' })
}

/** 查看详情 */
function handleDetail(item: Product) {
  uni.navigateTo({ url: `/pages-erp/product/product/detail/index?id=${item.id}` })
}
