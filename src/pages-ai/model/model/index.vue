      ...
      <view class="p-24rpx">
        ...
        >
          ...
          <view class="mt-20rpx flex justify-end gap-16rpx">
            <wd-button
              v-if="hasAccessByCodes(['ai:model:update'])"
            ...
            <wd-button
              v-if="hasAccessByCodes(['ai:model:delete'])"
...
/** 新增模型 */
function handleAdd() {
  uni.navigateTo({ url: '/pages-ai/model/model/form/index' })
}

/** 编辑模型 */
function handleEdit(item: ModelVO) {
  uni.navigateTo({ url: `/pages-ai/model/model/form/index?id=${item.id}` })
}

/** 删除模型 */
async function handleDelete(item: ModelVO) {
  try {
    await dialog.confirm({ title: '提示', msg: `确定要删除模型【${item.name}】吗？` })
  } catch {
    return
  }
